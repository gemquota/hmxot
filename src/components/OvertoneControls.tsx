...
const PRESETS = ['natural', 'bright', 'dark', 'bell', 'organ', 'metallic'];
...
export function OvertoneControls() {
  ...
  return (
    <div className="h-full flex flex-col gap-4 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-500/30">
      {/* Play/Pause Button */}
      <button
      ...
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Presets</h3>
        <div className="grid grid-cols-3 gap-2">
          {PRESETS.map(preset => (
            <button
              key={preset}
              onClick={() => setPreset(preset)}
              className="py-2 px-2 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-medium transition-all capitalize"
            >
              {preset}
            </button>
          ))}
        ...
        <div className="flex gap-2 mt-2">
          <button
            ...
            className="flex-1 py-2 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-xs font-medium transition-all"
          >
            🎲 Randomize
          </button>
          <button
            onClick={resetOvertones}
            className="flex-1 py-2 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-xs font-medium transition-all"
          >
            ↺ Reset
          </button>
        </div>
      ...
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Fundamental</h3>
        <input
          type="range"
          min="65"
          max="1046"
          ...
          {notes.map(note => (
            octaves.map(oct => (
              <button
                ...
                className="py-1 rounded bg-white/5 hover:bg-purple-500/30 text-[10px] transition-all"
              >
                {note}{oct}
              </button>
            ))
      ...
      {/* Overtone Sliders */}
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Overtones</h3>
      ...
      {/* Waveform */}
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Waveform</h3>
        <div className="grid grid-cols-4 gap-2">
      ...
      {/* Filter & Effects */}
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Filter & Effects</h3>
      ...
      {/* Quick Trigger */}
      <div className="glass rounded-xl p-3">
        <h3 className="text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wider">Quick Trigger</h3>
        <div className="grid grid-cols-7 gap-1">
          {['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4'].map(note => (
            <button
              key={note}
              onClick={() => trigger(note)}
              className="py-2 rounded-lg bg-white/5 hover:bg-purple-500/40 text-xs font-medium transition-all"
            >
              {note}
            </button>
          ))}
